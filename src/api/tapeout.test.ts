...
describe('TapeOut protocol adapter', () => {
  ...
  it('normalizes processor, circuit, task and miner owner feeds', async () => {
    vi.stubGlobal('fetch', vi.fn((input: RequestInfo | URL) => {
      ...
      if (url.endsWith('pod-mainnet.json')) return Promise.resolve(new Response(JSON.stringify({ chainId: 56, contracts: { factory: '0xfactory' }, cpus: { TapeOut: { address: '0xabc', multiplier: 2, fromBlock: 10 } }, circuits: [{ circuitId: 7, owner: '0xowner', cpu: 'TapeOut' }] })))
      ...
      if (url.endsWith('pod-taskbank.json')) return Promise.resolve(new Response(JSON.stringify({ meta: { onchain: 2, total: 3 }, tasks: [{ taskId: 1, name: 'Adder', kind: 'comb', onchain: true, refGates: 4 }] })))
    ...
    expect(snapshot.tasks[0]?.id).toBe(1)
    expect(snapshot.contracts.factory).toBe('0xfactory')
